        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                { 
                    ...
                    if($.isFunction(p.onReceivedData)){//hacked by yangfan 20120109
                    	data = p.onReceivedData(data);
                    }
            ...
            if (p.onMakeAjaxOptions) {
            	var res = p.onMakeAjaxOptions(ajaxOptions,p.newPage,p.pageSize);//hacked by yangfan 20120109
            	if(res == false){
            		g.toggleLoading.ligerDefer(g, 10, [false]);
            		$(".l-bar-btnload span", g.toolbar).removeClass("l-disabled");
            		return;
